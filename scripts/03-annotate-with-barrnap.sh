...
for assembly in `ls genomes-with-proportal-clade/*/*/Assembly/IMG_Data/*fna | grep -E "contigs.fna|assembled.fna"`; do

	outdir=`dirname $assembly`
	ecotype=`echo $outdir | cut -f2 -d\/`
	IMGgenomeID=`echo $outdir | cut -f3 -d\/`
	outname=$ecotype.$IMGgenomeID.`basename $assembly .fna`.rRNA.fna
